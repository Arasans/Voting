...
<section id="Card" class="pt-32">
    <div class="container flex flex-wrap justify-center">
        @if(session()->has('success'))
                <div class="max-w-lg container bg-green-100 rounded-lg py-5 px-6 mb-4 text-base text-green-700" role="alert">
                    <p class="text-center">{{ session('success') }}</p>
                </div>
            @endif
            @if(session()->has('Fail'))
                <div class="max-w-lg container bg-red-100 rounded-lg py-5 px-6 mb-4 text-base text-red-700" role="alert">
                    <p class="text-center">{{ session('Fail') }}</p>
                </div>
            @endif
        <div class="flex flex-wrap justify-center ">
            @foreach ($finalis as $item)
                
            <div id="{{ $item['nama'] }}" class="shadow-lg sm:mx-5 mt-10">
                ...
                <div class="w-full h-12">
                    <form action="/finalis" method="POST">
                        @csrf
                        <input value="{{ $item['id'] }}" type="hidden" name="json" id="json_callback">
